...
If adding as a submodule, or copying the library into your project dir directly, all you need
is [the header file](include/plot/plot.h):

```cpp
#include "<plot/plot.h>";
```

If you'd instead like to add `plot` as a CMake dependency, you can do so with FetchContent, via

```cmake
include(FetchContent)
FetchContent_Declare(
        plot
        GIT_REPOSITORY https://github.com/Signalsmith-Audio/plot
        GIT_TAG main
        GIT_SHALLOW ON
)
...
This creates an interface target you can then link your binary to, with `target_link_libraries`:

```cmake 
target_link_libraries(YourTarget PRIVATE signalsmith::plot)
```

The `include/` directory will now be added to your include search paths, so as above, to include it in your project:

```cpp
#include <plot/plot.h>
```

Examples can be built by passing `-DSIGNALSMITH_PLOT_EXAMPLES=1` to your cmake invocation - this will create a new
executable target called `plot-examples`.
...
When you create a sub-object (line, sub-plot, etc.), you are returned a reference. These elements are deliberately not
copyable, so if you use `auto` instead of `auto &` it will complain.
...
There's an [additional module](file:///Users/geraint/Development/plot/doc/html/group___heat-_map.html) in `heatmap.h`
